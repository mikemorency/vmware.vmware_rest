...
"""
Unit tests for appliance_localaccounts_globalpolicy_info module.
...
returns the global password policy for local accounts as a single dict.
"""
...
# ============================================================================
# Test GET Operations (Singleton returning the policy dict)
# ============================================================================
...
def test_get_policy(
    ...
    """Test getting the global password policy."""
    patch_create_client.return_value = mock_client
    mock_module = _run_module(
        patch_ansible_module, mock_client, module_args, 200, SAMPLE_POLICY
    )
    ...
    assert result["value"] == SAMPLE_POLICY
    # info is always a list; the singleton yields exactly one element.
    assert result["info"] == [SAMPLE_POLICY]
    # The policy dict has no MOID attribute, so no id should be reported.
...
def test_get_policy_not_found(
    ...
    """Test getting the policy when the endpoint returns 404."""
    patch_create_client.return_value = mock_client
...
class TestCheckMode:
    ...
    def test_get_check_mode(
        ...
        """Test getting the policy in check mode."""
        patch_create_client.return_value = mock_client
        mock_module = MagicMock()
        patch_ansible_module.return_value = mock_module
        mock_module.params = set_module_args(module_args)
        ...
        mock_client.get.return_value = _response(200, SAMPLE_POLICY)
...
class TestAPICallPath:
    ...
    def test_get_api_path(
        ...
        patch_create_client.return_value = mock_client
        _run_module(patch_ansible_module, mock_client, module_args, 200, SAMPLE_POLICY)
