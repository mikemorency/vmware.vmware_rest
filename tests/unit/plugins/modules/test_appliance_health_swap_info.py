...
"""
Unit tests for appliance_health_swap_info module.
...
from ansible_collections.vmware.vmware_rest.plugins.modules import (
    appliance_health_swap_info as module_under_test,
)
...
def test_get_swap_health_green(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting swap health when services are healthy."""
...
def test_get_swap_health_degraded(
    patch_create_client,
    ...
    """Test getting swap health for each non-green status value."""
    patch_create_client.return_value = mock_client
    mock_module = _run_module(
...
def test_get_swap_health_not_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting swap health when the endpoint returns 404."""
    patch_create_client.return_value = mock_client
...
class TestCheckMode:
    ...
    def test_get_check_mode(
        ...
        """Test getting swap health in check mode."""
        patch_create_client.return_value = mock_client
        mock_module = MagicMock()
        patch_ansible_module.return_value = mock_module
        mock_module.params = set_module_args(module_args)
...
class TestAPICallPath:
    ...
    def test_get_api_path(
        ...
        assert call_args[0][0] == "/appliance/health/swap"
...
class TestModuleConstants:
    ...
    def test_item_endpoint(self):
        ...
        assert module_under_test.ITEM_ENDPOINT == "/appliance/health/swap"

    def test_get_operation_config(self):
        """Test that the GET operation config targets the item endpoint."""
        assert module_under_test.GET_OPERATION.uri == "/appliance/health/swap"
